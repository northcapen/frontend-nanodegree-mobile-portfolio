...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
      ...
      // pizza image size depends only from pizza size (S, M, L). So, we can greatly simplify code here.
...
// Heavy modification DOM in loop isn't great idea. I put all pizzas in not DOM-bindind object (container), and attach container to DOM only once.
var pizzasContainer = document.createElement("div");
for (var i = 2; i < 100; i++) {
    pizzasContainer.appendChild(pizzaElementGenerator(i));
}
document.getElementById("randomPizzas").innerHTML = pizzasContainer.innerHTML;
...
function pizzaVerticalOffset(index) {
    // It is some heuristic for determination if we can see pizza on screen.
    // So, if cols = 8, pizzas with indexes 0..7 have vertical offset 0, indexes 8..15 - 256 and so on.
